...
class SchrodingerHPMNet(SchrodingerNet):
    def __init__(self, numLayers, numFeatures, numLayers_hpm, numFeatures_hpm, lb, ub, samplingX, samplingY, activation=torch.tanh, activation_hpm=F.relu):
        ...
        super().__init__(lb, ub, samplingX, samplingY, activation, noLayers=numLayers,
                         noFeatures=numFeatures, use_singlenet=True, ssim_windowSize=9, initLayers=True, useGPU=True)
    ...
    def init_layers_hpm(self):
        ...
        for _ in range(self.noLayers_hpm):
            self.lin_layers_hpm.append(
                nn.Linear(self.noFeatures_hpm, self.noFeatures_hpm))
    ...
    def hpm_loss(self, x, y, t, Ex_u, Ex_v):
        ...
        hpmLoss = torch.mean(f_u ** 2) + torch.mean(f_v ** 2) + \
            torch.mean((u - Ex_u) ** 2) + torch.mean((v - Ex_v) ** 2)
        return hpmLoss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--numfeatures", dest="numFeatures",
                        type=int, default=300)
    parser.add_argument("--numlayers_hpm",
                        dest="numLayers_hpm", type=int, default=4)
    parser.add_argument("--numfeatures_hpm",
                        dest="numFeatures_hpm", type=int, default=300)
    ...
    # create dataset
    ds = SchrodingerHPMEquationDataset(
        args.pData, coordinateSystem, args.numBatches, args.batchsize, shuffle=True, useGPU=True)
    # Partition dataset among workers using DistributedSampler
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        ds, num_replicas=hvd.size(), rank=hvd.rank())
    train_loader = torch.utils.data.DataLoader(
        ds, batch_size=1, sampler=train_sampler)

    model = SchrodingerHPMNet(args.numLayers, args.numFeatures, args.numLayers_hpm,
                              args.numFeatures_hpm, ds.lb, ds.ub, 5, 5, activation).cuda()
        ...
        while (l_loss > args.t_ic or epoch < 30000):
            ...
            if (epoch % 1000 == 0) and log_writer:
                ...
                print("[%d] IC loss: %.4e [%.2fs]" %
                      (epoch, l_loss, time.time() - start_time))
                log_writer.add_scalar("loss_ic", l_loss, epoch)

                writeValidationLoss(0, model, epoch, log_writer,
                                    coordinateSystem, identifier="PT")
                writeIntermediateState(
                    0, model, epoch, log_writer, coordinateSystem, identifier="PT")
                writeValidationLoss(
                    500, model, epoch, log_writer, coordinateSystem, identifier="PT")
                writeIntermediateState(
                    500, model, epoch, log_writer, coordinateSystem, identifier="PT")
                writeValidationLoss(
                    1000, model, epoch, log_writer, coordinateSystem, identifier="PT")
                writeIntermediateState(
                    1000, model, epoch, log_writer, coordinateSystem, identifier="PT")
    ...
    while(l_loss > args.t_pde):
        ...
        if (epoch % 1000 == 0) and log_writer:
            ...
            print("[%d] PDE loss: %.4e [%.2fs] saved" %
                  (epoch, loss.item(), time.time() - start_time))
                  
            write_coefficients(model, ds, epoch, coordinateSystem, 0, log_writer)
            write_coefficients(model, ds, epoch, coordinateSystem, 250, log_writer)
            write_coefficients(model, ds, epoch, coordinateSystem, 500, log_writer)
            write_coefficients(model, ds, epoch, coordinateSystem, 750, log_writer)
            write_coefficients(model, ds, epoch, coordinateSystem, 1000, log_writer)
	    
            write_dependencies(model, ds, epoch, log_writer, coordinateSystem, time=0)
            write_dependencies(model, ds, epoch, log_writer, coordinateSystem, time=250)
            write_dependencies(model, ds, epoch, log_writer, coordinateSystem, time=500)
            write_dependencies(model, ds, epoch, log_writer, coordinateSystem, time=750)
            write_dependencies(model, ds, epoch, log_writer, coordinateSystem, time=1000)
	    
            writeIntermediateState(
                0, model, ds, epoch, log_writer, coordinateSystem, identifier="PDE")
            writeIntermediateState(
                500, model, ds, epoch, log_writer, coordinateSystem, identifier="PDE")
            writeIntermediateState(
                1000, model, ds, epoch, log_writer, coordinateSystem, identifier="PDE")

            writeValidationLoss(0, model, ds, epoch, log_writer,
                                coordinateSystem, identifier="PDE")
            writeValidationLoss(500, model, ds, epoch, log_writer,
                                coordinateSystem, identifier="PDE")
            writeValidationLoss(1000, model, ds, epoch, log_writer,
                                coordinateSystem, identifier="PDE")

            sys.stdout.flush()

            log_writer.add_histogram(
                'First Layer Grads', model.lin_layers_hpm[0].weight.grad.view(-1, 1), epoch)
